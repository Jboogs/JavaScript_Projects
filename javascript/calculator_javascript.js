 //Calculator JS by JP
 //This will create an object to hold values

 const Calculator = {
     Display_Value: "0", //displays 0 on screen at start
...
function Input_Digit(digit) {
    ...
    //display value to keythat was clicked
    if (Wait_Second_Operand == true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //overwrites the current display value if that value is 0
...
function Input_Decimal(dot) {
    //this ensures the accuidental clicking of the decimal doesnt cause any bugs
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //if display value does not contain a decimal point 
        //we want to include a data point
